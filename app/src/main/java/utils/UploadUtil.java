...
public class UploadUtil {
    ...
    private final static String THREAD_RESULT_OBJECT="THREAD_RESULT_OBJECT";
    ...
    public void submitAll(ArrayList<String> filePath){
        ...
        for(int i=0;i<threadCount;i++){//模拟生成任务线程
            ...
            executor.submit(new UploadFile(downLatch,filePath.get(i),new OnThreadResultListener(){
                ...
                @Override
                public void onFinish(AVFile avFile) {
                    Message msg=Message.obtain();
                    msg.obj=avFile;
                    msg.setData(bundle);
                    msg.what=THREAD_FINISH_CODE;
            ...
            if(uploadUtil!=null){
                ...
                switch (msg.what){
                    ...
                    case THREAD_FINISH_CODE:
                        data=msg.getData();
                        position=data.getInt(THREAD_POSITION);
                        uploadUtil.uploadListener.onThreadFinish(position, (AVFile) object);
                        break;
